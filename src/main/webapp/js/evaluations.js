...
async function chargerEvaluations(evenementId) {
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE}/api/evaluations?evenementId=${evenementId}`,
      {
        method: "GET",
        credentials: "include",
        headers: {
...
function afficherEvaluations(evaluations, evenementId, moyenne, total) {
  ...
  const evaluationParticipant = evaluations.find(
            ...
            <div class="card-body">
                <div class="row align-items-center">
                    <div class="col-md-4 text-center border-end">
                        <div class="display-4 fw-bold text-primary">${moyenne.toFixed(
                          1
                        )}</div>
                        <div class="text-warning fs-4">${getStarsHTML(
                          moyenne
                        )}</div>
                        <div class="text-muted small">${total} évaluation${
    total > 1 ? "s" : ""
  }</div>
                    </div>
                    <div class="col-md-8">
  ...
  if (evaluations.length === 0) {
    html += `
            <div class="text-center py-4">
                ...
                ${
                  isInscrit
                    ? '<p class="text-muted small">Soyez le premier à évaluer !</p>'
                    : ""
                }
            </div>
        `;
  ...
  for (let i = 5; i >= 1; i--) {
    ...
    html += `
            <div class="d-flex align-items-center mb-2">
                <span class="me-2 small" style="width: 60px;">${i} étoile${
      i > 1 ? "s" : ""
    }</span>
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-primary btn-sm">
                            <i class="bi bi-star me-1"></i>${
                              isModification ? "Modifier" : "Publier"
                            } l'évaluation
                        </button>
                        ${
                          isModification
            ...
            <div class="d-flex justify-content-between align-items-start mb-2">
                <div class="d-flex align-items-center">
                    <div class="avatar-circle bg-primary text-white me-2">
                        ${
                          evaluation.participantNom
                            ? evaluation.participantNom.charAt(0).toUpperCase()
                            : "U"
                        }
                    </div>
                    <div>
                        <div class="fw-semibold">${escapeHtml(
                          evaluation.participantNom || "Utilisateur"
                        )}</div>
                        <div class="text-warning small">${getStarsHTML(
                          evaluation.note
                        )}</div>
                    </div>
                </div>
...
function attacherEvaluationListeners(evenementId) {
  ...
  // Bouton supprimer
  document
    .querySelector(".btn-delete-evaluation")
...
async function soumettreEvaluation(evenementId, note, texte) {
  ...
  try {
    ...
    if (response.ok && result.success) {
      ...
      // Mettre à jour immédiatement les statistiques locales (nombre d'évaluations)
      window.dashboardAPI?.incrementEvaluations?.();
    } else {
      window.dashboardAPI?.showToast(
    ...
    if (submitBtn) {
      ...
      submitBtn.innerHTML =
        '<i class="bi bi-star me-1"></i>Publier l\'évaluation';
    }
  }
}
...
async function supprimerEvaluation(evaluationId, evenementId) {
  try {
    const response = await fetch(
      `${API_BASE}/api/evaluations/${evaluationId}`,
      {
        method: "DELETE",
        credentials: "include",
        headers: {
...
// ========== Ouverture Modal Évaluation (Post-Événement) ==========
function openAddEvaluationModal(eventId) {
  const modal = document.getElementById("modalAddEvaluation");
  if (!modal) {
    console.error("Modal évaluation non trouvée");
    return;
  }
  ...
  const eventInfo = document.getElementById("evaluationEventInfo");
  if (eventInfo) {
    eventInfo.innerHTML = `
...
window.openAddEvaluationModal = openAddEvaluationModal;
