...
function openInscriptionModal(eventId) {
  ...
  document.getElementById("inscriptionQuantite").max = Math.min(
    10,
    event.capaciteDisponible
  );
...
document
  .getElementById("btnConfirmInscription")
  ?.addEventListener("click", async function () {
    ...
    const quantite = parseInt(
      document.getElementById("inscriptionQuantite").value
    );
    ...
    btn.disabled = true;
    btn.innerHTML =
      '<span class="spinner-border spinner-border-sm me-1"></span>Inscription...';
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        const event = new CustomEvent("inscriptionUpdated", {
          detail: { inscriptionId: result.data?.inscriptionId, eventId: eventId }
        });
...
function confirmCancelInscription(inscriptionId) {
  ...
  const modal = new bootstrap.Modal(
    document.getElementById("modalConfirmCancel")
  );
  modal.show();
}
  ...
  ?.addEventListener("click", async function () {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/participant/inscriptions/${inscriptionId}`,
        {
          method: "DELETE",
          credentials: "include",
          headers: {
      ...
      if (response.ok && result.success) {
        ...
        // Déclencher un événement pour mettre à jour le calendrier
        const event = new CustomEvent("inscriptionUpdated", {
          detail: { inscriptionId: inscriptionId, action: "cancelled" }
        });
        document.dispatchEvent(event);
...
let eventDetailsModalInstance = null;
let eventDetailsModalInitialized = false;

function getEventDetailsModalInstance() {
  const modalElement = document.getElementById("modalEventDetails");
  ...
  if (!eventDetailsModalInstance) {
    ...
    eventDetailsModalInstance = bootstrap.Modal.getOrCreateInstance(modalElement);
  }

  if (!eventDetailsModalInitialized) {
    modalElement.addEventListener("hidden.bs.modal", () => {
      // Nettoyage manuel au cas où Bootstrap laisserait un backdrop résiduel
    ...
    eventDetailsModalInitialized = true;
  }

  return eventDetailsModalInstance;
}
...
function openEventDetailsModal(eventId) {
  ...
  // Trouver l'événement dans les événements disponibles
  let event = dashboardData.evenementsDisponibles?.find(
    (e) => e.evenementId == eventId
  );

  // Si non trouvé, chercher dans les inscriptions (pour les événements passés)
  if (!event) {
    // Chercher dans les inscriptions actives
    let inscription = null;
    const inscriptionsActives =
      window.dashboardAPI?.getInscriptionsActives?.() || [];
    inscription = inscriptionsActives.find((i) => i.evenementId == eventId);

    // Chercher dans les inscriptions passées
    if (!inscription) {
      const inscriptionsPassees =
        window.dashboardAPI?.getInscriptionsPassees?.() || [];
      inscription = inscriptionsPassees.find((i) => i.evenementId == eventId);
    }

    // Chercher dans les inscriptions annulées
    if (!inscription) {
      const inscriptionsAnnulees =
        window.dashboardAPI?.getInscriptionsAnnulees?.() || [];
      inscription = inscriptionsAnnulees.find((i) => i.evenementId == eventId);
    }

    // Transformer l'inscription en objet événement
    if (inscription) {
      const dateFin = new Date(inscription.evenementDateFin);
      const now = new Date();
      const isEvenementPasse = dateFin < now;

      event = {
        evenementId: inscription.evenementId,
        titre: inscription.evenementTitre,
        description: inscription.evenementDescription,
        dateDebut: inscription.evenementDateDebut,
        dateFin: inscription.evenementDateFin,
        lieu: inscription.evenementLieu,
        capacite: inscription.evenementCapacite || 0,
        capaciteDisponible: inscription.capaciteDisponible || 0,
        statut: "PUBLIE", // Les événements passés sont toujours PUBLIE
        noteMoyenne: 0,
        nombreEvaluations: 0,
        nombreInscrits: inscription.nombreInscrits || 0,
        nombreCommentaires: 0,
        statutInscription: inscription.statut,
        isEvenementPasse: isEvenementPasse,
      };
    }
  }
  ...
  if (event.statutInscription) {
    const badges = {
      ACCEPTEE:
        '<span class="badge badge-inscrit fs-6">✓ Vous êtes inscrit</span>',
                        ...
                        <div class="col-md-6">
                            <div class="d-flex align-items-start">
                                ...
                                <div>
                                    ...
                                    <div>${dateDebut.toLocaleDateString(
                                      "fr-FR",
                                      {
                                        weekday: "long",
                                        day: "numeric",
                        ...
                        <div class="col-md-6">
                            <div class="d-flex align-items-start">
                                ...
                                <div>
                                    ...
                                    <div>${event.nombreInscrits}/${
    event.capacite
  } inscrits</div>
                                    <div class="text-${
                                      event.capaciteDisponible <= 10
                                        ? "danger"
                                        : "success"
                                    } small">
                                        ${
  ...
  // Afficher le modal
  const modal = getEventDetailsModalInstance();
  if (!modal) return;
  // S'assurer qu'aucun ancien backdrop n'est présent avant l'ouverture
  document
    .querySelectorAll(".modal-backdrop")
    .forEach((backdrop) => backdrop.remove());
  document.body.classList.remove("modal-open");
