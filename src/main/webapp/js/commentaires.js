    ...
    if (commentaires.length === 0) {
        html += `
            <div class="text-center py-4">
                ...
                ${isInscrit ? '<p class="text-muted small">Soyez le premier à commenter !</p>' : ''}
            </div>
        `;
    } else {
...
function creerCommentaireHTML(commentaire) {
    ...
    return `
        <div class="commentaire-item border-bottom py-3" data-commentaire-id="${commentaire.commentaireId}">
            <div class="d-flex justify-content-between align-items-start mb-2">
                <div class="d-flex align-items-center">
                    <div class="avatar-circle bg-primary text-white me-2">
                        ${commentaire.participantNom ? commentaire.participantNom.charAt(0).toUpperCase() : 'U'}
                    </div>
                    <div>
                        <div class="fw-semibold">${escapeHtml(commentaire.participantNom || 'Utilisateur')}</div>
...
function attacherCommentaireListeners(evenementId) {
    ...
    form?.addEventListener('submit', async function(e) {
        ...
        if (!texte) {
            window.dashboardAPI?.showToast('Le commentaire ne peut pas être vide', 'warning');
            return;
        }
...
async function ajouterCommentaire(evenementId, texte) {
    ...
    try {
        ...
        if (response.ok && result.success) {
            window.dashboardAPI?.showToast('Commentaire publié avec succès ! ✓', 'success');
            
            // Recharger les commentaires
            await chargerCommentaires(evenementId);
            
        } else {
            window.dashboardAPI?.showToast(result.message || 'Erreur lors de la publication', 'error');
        }
...
async function supprimerCommentaire(commentaireId, evenementId) {
    try {
        const response = await fetch(`${API_BASE}/commentaires/${commentaireId}`, {
            method: 'DELETE',
            credentials: 'include',
            headers: {
                'Accept': 'application/json'
